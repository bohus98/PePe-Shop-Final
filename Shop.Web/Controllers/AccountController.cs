...
namespace Shop.Web.Controllers
{
    public class AccountController : Controller
    {
        ...
        public async Task<IActionResult> Login(AccountLoginModel login)
        {
            ...
            ModelState.AddModelError("Neplatny vstup", "Uzivatelske meno alebo heslo je nespravne");
            return View(login);
        }
        ...
        public async Task<IActionResult> SaveProfile(AccountSettingsModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                    ...
                    || !string.IsNullOrEmpty(model.NewPasswordConfirmation))
                {
                    if (string.IsNullOrEmpty(model.OldPassword))
                    {
                        ModelState.AddModelError("Stare heslo", "Zadajte sucasne heslo na zmenu stareho");
                        return View("Settings", model);
                    }
                    if (string.IsNullOrEmpty(model.NewPassword))
                    {
                        ModelState.AddModelError("Nove heslo", "Zadajte nove heslo na zmenu stareho");
                        return View("Settings", model);
                    }
                    ...
                    if (!result.Succeeded)
                    { 
                        ModelState.AddModelError("Stare heslo", "nespravne heslo, zadajte sucasne hesl na zmenu stareho");
                        return View("Settings", model);
                    }
                }
